...
object ExampleData {
    val airPassengers = ExampleDataset(
        id = "air_passengers",
...
""".trimIndent(),
    ...
    val lateNightSnacks = ExampleDataset(
        id = "user_1",
...
2026-01-09,3,25.26
""".trimIndent(),
        idColumn = "id", // default placeholder since csv has no id column
        timestampColumn = "date",
        targetColumn = "daily_spend_usd",
        defaultPredictionLength = "5",
        defaultQuantiles = "0.5, 0.9" // Spending limits focus
    )

    val cloudCallEvents = ExampleDataset(
        id = "server_cluster_a",
...
""".trimIndent(),
        ...
        defaultQuantiles = "0.8, 0.9, 0.95, 0.99" // Anomaly detection focus
    )

    val signalStrength = ExampleDataset(
        id = "elevator_b2",
...
""".trimIndent(),
        idColumn = "id", // No explicit ID column in CSV, parser might default or we could use 'slot' but data is uniform
        timestampColumn = "timestamp",
        targetColumn = "condition_score_0_10",
        defaultPredictionLength = "48", // 48 hours? Data seems hourly/sparse.
                 // Actually data is 3 times a day (morning, lunch, night). 48 steps = 16 days.
        defaultQuantiles = "0.1, 0.5, 0.9"
    )

    val hourlyDemand = ExampleDataset(
        id = "store_gangnam",
...
""".trimIndent(),
        ...
        defaultPredictionLength = "24", // 24 hours
        defaultQuantiles = "0.1, 0.5, 0.9"
    )

     val sleepTracker = ExampleDataset(
        id = "user_health",
...
""".trimIndent(),
        ...
        defaultPredictionLength = "7", // Weekly outlook
        defaultQuantiles = "0.1, 0.5" // Focus on min sleep
    )

    val nasdaqYearly = ExampleDataset(
        id = "nasdaq_12m",
...
""".trimIndent(),
    ...
    val nasdaqHalfYearly = ExampleDataset(
        id = "nasdaq_6m",
