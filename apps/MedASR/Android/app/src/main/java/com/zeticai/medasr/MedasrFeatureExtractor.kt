...
object MedasrFeatureExtractor {
    ...
    fun extract(samples: FloatArray, sampleRate: Int, config: Config = Config()): Features {
        ...
        val numFrames = 1 + ((samples.size - config.winLength) / config.hopLength)
    ...
    private fun hannWindow(n: Int): FloatArray {
        ...
        // Periodic window (standard for FFT/PyTorch): denom = n
        // Symmetric window (standard for Filter design): denom = n - 1
        val denom = n.toFloat() 
        for (i in 0 until n) {
