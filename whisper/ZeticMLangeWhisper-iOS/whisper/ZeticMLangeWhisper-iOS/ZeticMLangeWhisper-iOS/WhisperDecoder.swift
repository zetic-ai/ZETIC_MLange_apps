...
class WhisperDecoder {
    ...
    init() {
        model = (try? ZeticMLangeModel(tokenKey: PrivateValues.personalKey, name: WhisperDecoder.modelKey))!
    }
    ...
    private func decodeStep(decoderTokenIds: [Int32], encoderOutput: Data, decoderAttentionMask: [Int32]) -> [Float] {
        return autoreleasepool {
            ...
            try? model.run(inputs: [
                Tensor(data: decoderTokenIdsData, dataType: BuiltinDataType.int8, shape: [decoderTokenIdsData.count]),
                Tensor(data: encoderOutput, dataType: BuiltinDataType.int8, shape: [encoderOutput.count]),
                Tensor(data: decoderAttentionMaskData, dataType: BuiltinDataType.int8, shape: [decoderAttentionMaskData.count]),
            ])
